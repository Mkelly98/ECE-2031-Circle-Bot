...
Targeting:
	LOAD Zero
	Store BestX ; Zero out variables
	Store BestY
	Store BestT
	ADD TwoToTheFifteenthMinusOne ; Set some stupid huge distance as BestD
	Store BestD
	Out LEDs
	
	LOAD Zero
	ADDI 3
	Call TurnAndStop ; Turn to 3 before starting sweep
	
TargetingLoop:
	IN Theta
	OUT SSEG1;
	ADDI 20 ; Attempt constant slow rotation - this constant was chosen arbitraily
	STORE DTheta
	IN Theta ; Check if rotation has been completed
	ADDI -357 ; Not a full rotation, but pretty good
	JPOS EndTargeting 
	
	IN DIST5 ; Take a distance reading
	STORE Temp
	Load Temp
	Sub BestD
	JPOS TargetingLoop ; This is not our best find, skip to next iteration
	
	Load Temp ; If we've reached this point Temp holds out new best
	Store BestD
	Out LEDs
	IN Theta ; Calculate and store BestX
	ADDI 270 ; Subtract 90 as the sensor is on the side of the robot
	STORE BestT
	OUT SSEG2;
	;CALL CosX
	;Add XPos
	;Store BestX
	;Load BestD ; Do the same for BestY
	;Store Temp
	;IN Theta
	;ADDI 270
	;CALL SinX
...
SinX:
    ; Return approximation of Sin(x) times A.
    ; A is stored in temp by the caller.
    ; X is in the AC when called.
    CALL Mod360
    OUT Trig
    IN Trig
    STORE M16SA
...
CosX:
	; Return approximation of Cos(x) times A.
    ; A is stored in temp by the caller.
    ; X is in the AC when called.
    ADDI 90
    CALL SinX
...
; Subroutine that turns to the angle specified in AC and then waits to come to a complete stop
TurnAndStop:
	CALL MOD360
	Store DTheta;
TurnAndStopLoop:
	...
	SUB DTheta
	CALL Abs
	ADDI -3 ; Absolute value of position error must be less then 3 degrees
	JPOS TurnAndStopLoop
	RETURN
...
Nine:     DW 9
Ten:      DW 10
TwoToTheFifteenthMinusOne: DW 32767
...
EnRight:	  DW &B00100000

; Tracking variables
BestX:	DW 0 ; X coordinate of best pylon in odometry space
BestY:  DW 0 ; Y coordinate of best pylon in odometry space
BestD:  DW 0 ; Distance to best pylon in mm
BestT:  DW 0 ; Best theta in degree
...
TRIG:	  EQU &H0B  ; trig lookup table. Returns fixed-point approximation of sin(x)